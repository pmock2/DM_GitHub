...
public class Server {	
	...
	public static List<Referee> getActiveReferees() {
		List<Referee> activeReferees = new LinkedList<>();
		for (Map.Entry<Integer, Referee> entry : referees.entrySet()) {
			Referee referee = entry.getValue();
			if (referee.isActive() && !referee.isSuperReferee) {
				activeReferees.add(referee);
			}
		}
